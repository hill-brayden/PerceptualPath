...
def create_memory_cloud(
    ...
    for i in range(len(points)):
        ...
        persistence_attr.data[i].value = (
            persistence_values[i]
        )

        first_seen_attr.data[i].value = (
            first_seen_values[i]
        )

        last_seen_attr.data[i].value = (
            last_seen_values[i]
        )
...
class PP_OT_run_experience(bpy.types.Operator):
    ...
    bl_description = (
        ...
        "and accumulate visible geometry"
    )
    ...
    def execute(self, context):
        ...
        for step_number, index in enumerate(
            ...
            # ---------------------------------------------------
# GROUP THIS VIEWPOINT'S RAYS BY MEMORY CELL
# ---------------------------------------------------
...
# ---------------------------------------------------
# RECORD EACH CELL ONLY ONCE FOR THIS VIEWPOINT
# ---------------------------------------------------
...
def configure_analysis_material(mode):
    ...
    if mode == 'PLAIN':
        ...
        middle = ramp.color_ramp.elements.new(
            0.5
        )
