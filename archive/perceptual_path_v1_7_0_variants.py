...
def ensure_variant_parent_collection(scene):
    parent = bpy.data.collections.get(PP_VARIANT_PARENT)

    if parent is None:
        parent = bpy.data.collections.new(PP_VARIANT_PARENT)
        scene.collection.children.link(parent)

    return parent


def set_variant_visibility(scene, active_index):
    source = bpy.data.objects.get(scene.pp_mutable_object)
    parent = bpy.data.collections.get(PP_VARIANT_PARENT)

    if source is not None:
        source.hide_set(active_index != 0)
        source.hide_render = (active_index != 0)

    if parent is None:
        return

    for child in parent.children:
        variant_index = int(child.get("pp_variant_index", -1))
        visible = (variant_index == active_index)
...
class PP_OT_set_mutable_object(bpy.types.Operator):

    bl_idname = "pp.set_mutable_object"
    bl_label = "Set Selected as Mutable"

    def execute(self, context):
        obj = context.active_object

        if obj is None or obj.type not in {'MESH', 'CURVE'}:
            self.report({'ERROR'}, "Select a mesh or curve first")
            return {'CANCELLED'}

        context.scene.pp_mutable_object = obj.name
        self.report({'INFO'}, f"Mutable object: {obj.name}")
        return {'FINISHED'}


class PP_OT_generate_variants(bpy.types.Operator):

    bl_idname = "pp.generate_variants"
    ...
    def execute(self, context):
        ...
        if source is None:
            self.report({'ERROR'}, "Set a mutable object first")
            return {'CANCELLED'}

        clear_variant_collections()
        parent = ensure_variant_parent_collection(scene)

        rng = random.Random(scene.pp_variant_seed)
        count = max(1, scene.pp_variant_count)
        source_matrix = source.matrix_world.copy()

        for index in range(1, count + 1):
            child = bpy.data.collections.new(f"PP_VARIANT_{index:02d}")
            child["pp_variant_index"] = index
            ...
            duplicate.name = f"{source.name}_V{index:02d}"
            child.objects.link(duplicate)
            duplicate.matrix_world = source_matrix.copy()

            duplicate.location.x += rng.uniform(
                -scene.pp_variant_move_x,
                scene.pp_variant_move_x
            )

            duplicate.location.y += rng.uniform(
                -scene.pp_variant_move_y,
                scene.pp_variant_move_y
            )

            duplicate.rotation_euler.z += math.radians(
                rng.uniform(
            ...
            duplicate["pp_variant_index"] = index
            duplicate["pp_source_object"] = source.name

        scene.pp_active_variant = 0
        set_variant_visibility(scene, 0)

        self.report({'INFO'}, f"Generated {count} variants")
        return {'FINISHED'}


class PP_OT_activate_variant(bpy.types.Operator):

    bl_idname = "pp.activate_variant"
    bl_label = "Activate Variant"
    ...
    def execute(self, context):
        ...
        if (
            scene.pp_active_variant > 0
            and bpy.data.collections.get(PP_VARIANT_PARENT) is None
        ):
            self.report({'ERROR'}, "Generate variants first")
            return {'CANCELLED'}

        set_variant_visibility(scene, scene.pp_active_variant)

        # Changing geometry invalidates old route and cloud.
        ...
        self.report(
            ...
            (
                ...
                else f"Variant {scene.pp_active_variant:02d} active"
            )
        )
...
class PP_OT_clear_variants(bpy.types.Operator):

    bl_idname = "pp.clear_variants"
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, "Generated variants cleared")
        return {'FINISHED'}
...
class PP_PT_main_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.label(text="ITERATION")

        mutable_name = (
            scene.pp_mutable_object
            if scene.pp_mutable_object
            else "None"
        )

        layout.label(text=f"Mutable: {mutable_name}")

        layout.operator(
            "pp.set_mutable_object",
            icon='EYEDROPPER'
        )

        layout.prop(scene, "pp_variant_count", text="Variant Count")
        layout.prop(scene, "pp_variant_move_x", text="Move X ±")
        layout.prop(scene, "pp_variant_move_y", text="Move Y ±")
        layout.prop(scene, "pp_variant_rotation", text="Rotate ± Degrees")
        layout.prop(scene, "pp_variant_seed", text="Seed")
        ...
        layout.label(
            text="0 = Original; activate then regenerate path",
            icon='INFO'
        )
...
def register():
    ...
    bpy.types.Scene.pp_mutable_object = (
